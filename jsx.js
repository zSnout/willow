...
import { createEffect, isAccessor, } from "./primitives.js";
var Bindable;
(function (Bindable) {
...
function setStyles(element, value) {
    if (isAccessor(value)) {
        ...
        for (const key in value) {
            const val = value[key];
            if (isAccessor(val)) {
                addNodeEffect(element, () => (element.style[key] = val()), {
                    name: `css style.${key}`,
                });
            }
            else {
...
export function h(tag, props, ...children) {
    if (typeof tag === "string") {
        ...
        appendReactive(el, children);
        for (const key in props) {
            const value = props[key];
            if (el instanceof HTMLElement &&
                (key === "class" || key === "className")) {
                if (isAccessor(value)) {
                    addNodeEffect(el, () => (el.className = fromClassLike(value())), { name: "className" });
                }
                else {
    ...
    throw new TypeError(`willow.h must be passed a tag name or function, but was passed a ${typeof tag}`);
}
(function (h) {
    function f({ children }) {
